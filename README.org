...
=FlowAnimation= and =Manim= depend on system libraries for text rendering. Install them before running any animated content:
...
| =background=       | Custom slide background (see [[*Slide backgrounds]])              |
...
When =background= is set and =content_kind= is not, the content text color is chosen automatically
